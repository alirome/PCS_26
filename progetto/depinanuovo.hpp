...
// ============================================================================
// FASE 2: RICERCA DEL CICLO MINIMO (Lifting Graph + Dijkstra)
// ============================================================================
inline std::pair<std::vector<int>, struttura_cicli<int>> trova_ciclo_minimo(
    const unidirected_graph<int>& G, const std::vector<int>& S_i) 
{
    int m = G.all_edges().size();
    int V_offset = *G.all_nodes().rbegin() + 1; 
    ...
    std::vector<int> maschera_ottima(m, 0);
    ...
    for (int nodo_start : G.all_nodes()) {
        ...
        while (!q.empty() && !trovato) {
            ...
            for (int vicino : albero.neighbours(cur)) {
                if (padre.find(vicino) == padre.end()) {
                    padre[vicino] = cur;
                    if (vicino == target) { trovato = true; break; }
                    q.push(vicino);
                }
            }
        }
        ...
        for (size_t i = 0; i + 1 < cammino.size(); ++i) {
            ...
            if (G.all_edges().count(arco)) {
                maschera_ottima[G.edge_number(arco)] ^= 1;
...
// ============================================================================
// FASE 3: ALLINEAMENTO TOPOLOGICO (A PROVA DI BOMBA)
// ============================================================================
inline void orienta_maglie(std::vector<struttura_cicli<int>>& maglie) {
    ...
    while (modificato) {
        ...
        for (size_t i = 1; i < maglie.size(); ++i) {
            if (orientata[i]) continue; // Se è già sistemata, passiamo oltre

            // La confrontiamo con tutte le maglie GIA' SISTEMATE
            for (size_t j = 0; j < maglie.size(); ++j) {
                ...
                // Controlliamo ogni arco della maglia da sistemare (i)
                for (size_t ki = 0; ki + 1 < maglie[i].nodes.size(); ++ki) {
                    int ui = maglie[i].nodes[ki], vi = maglie[i].nodes[ki+1];

                    // Controlliamo ogni arco della maglia buona (j)
                ...
                // Appena troviamo un punto in comune, prendiamo una decisione
                if (trovato_arco_comune) {
                    if (sbattono) {
                        // Invertiamo tutto il percorso della maglia i!
                        std::reverse(maglie[i].nodes.begin(), maglie[i].nodes.end() - 1);
                        maglie[i].nodes.back() = maglie[i].nodes.front(); // Chiudiamo il cerchio
                    }
                    orientata[i] = true;
                    modificato = true;
                    break; // Passiamo alla prossima maglia 'i'
                }
            }
        }
    }
...
// ============================================================================
// FASE 4: ALGORITMO DI DE PINA PRINCIPALE
// ============================================================================
inline std::vector<struttura_cicli<int>> de_pina_from_scratch(const unidirected_graph<int>& G, int nodo_sorgente) {
    
    // 1. Albero Spanning per trovare i generatori (S_i)
    lifo<int> pila;
    unidirected_graph<int> T = graph_visit(G, nodo_sorgente, pila);
    ...
    // 2. Ciclo di ortogonalizzazione di De Pina
    for (int i = 0; i < k; ++i) {
    ...
    // 3. Sistemazione finale: imponiamo la coerenza di rotazione
