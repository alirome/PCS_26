...
int main() {
	...
	quicksort_veloce(stringa, 0, stringa.size()-1, soglia);
    ...
    for (int i=0; i <100; i++){
	    ...
	    quicksort_veloce(vi, 0, vi.size()-1, soglia);
	    if (!is_sorted(vi)){
		    std::cerr<< "ordinamento fallito" << std::endl;
			return EXIT_FAILURE;
	    }
		...
		quicksort_veloce(vf, 0, vf.size()-1, soglia);
		if (!is_sorted(vf)) {
			std::cerr << "Ordinamento fallito" << std::endl;
			return EXIT_FAILURE;
		}
		...
		quicksort_veloce(vd, 0, vd.size()-1, soglia);
		if (!is_sorted(vd)) {
			std::cerr << "Ordinamento fallito" << std::endl;
			return EXIT_FAILURE;
		}
